...
router.get('/product', middleware, async (req, res) => {
  ...
  if (queryString === undefined || queryString.toLowerCase() === 'desc') {
    sort = 'desc'
  ...
  try {
    const listData = await Products.findAll({
      attributes: ['productId', 'productName', 'comment', 'price', 'status', 'updatedAt'],
      include: [{ model: UsersData, attributes: ['name'] }],
